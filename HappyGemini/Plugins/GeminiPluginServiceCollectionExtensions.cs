...
public static class GeminiPluginServiceCollectionExtensions
{
        ...
        IConfiguration configuration)
    {
        ...
        GeminiPluginOptions options = configuration
            .GetSection(GeminiPluginOptions.SectionName)
            .Get<GeminiPluginOptions>()
            ?? new GeminiPluginOptions();

        if (string.IsNullOrWhiteSpace(options.PluginDirectory))
        {
            throw new InvalidOperationException(
                $"{GeminiPluginOptions.SectionName}:PluginDirectory " +
                "must not be empty.");
        }
        ...
        IReadOnlyList<GeminiPluginDescriptor> descriptors =
            discovery.Discover(options.PluginDirectory);

        foreach (GeminiPluginDescriptor descriptor in descriptors)
        {
            GeminiLoadedPlugin loaded =
